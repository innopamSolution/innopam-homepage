...
export default function Footer() {
  ...
  return (
    <footer className="relative bg-black flex flex-col gap-[40px] items-center pb-[36px] pt-[80px] px-6 md:px-[80px]">
      {/* Main content — stack on mobile, side-by-side on desktop */}
      <div className="w-full max-w-[1280px] flex flex-col lg:flex-row items-start justify-between gap-10 lg:gap-0 lg:px-[40px]">

        {/* Left: CTA */}
        <div className="flex flex-col gap-[35px] lg:w-[526px] text-left">
          <p
            className="font-pretendard font-normal text-white text-[18px] md:text-[24px]"
            style={{ lineHeight: "43.2px", letterSpacing: "-0.48px" }}
          >
            {t(
          ...
          <button
            ...
            className="brand-gradient self-start inline-flex items-center text-white font-pretendard font-bold text-[13px] md:text-[14px] tracking-[1.3px] uppercase px-[36px] md:px-[54px] py-[12px] md:py-[15px] rounded-full hover:opacity-90 transition-opacity"
          >
            {t('문의하기', 'Contact Us')}
          </button>
        </div>
        ...
        {/* Right: Contact info */}
        <div className="flex flex-col gap-8 lg:gap-0 lg:h-[345px] lg:justify-between lg:w-[490px] text-left">
          {/* Address */}
          <div>
      ...
      {/* Bottom bar */}
      <div className="flex flex-col gap-[36px] w-full max-w-[1280px]">
        <div className="h-px bg-[rgba(255,255,255,0.2)] w-full" />
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-4 md:gap-0">
          <img src={asset('assets/logo-footer-new.svg')} alt="Innopam" className="h-[37px] w-auto object-contain" />
